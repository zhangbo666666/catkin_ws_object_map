...
void MinBoxObjectBuilder::computePolygon2dxy(ObjectPtr obj) {
    ...
    // after setDefaultValue
    std::cout << "after setDefaultValue " << std::endl;
    std::cout <<"l: " << obj->length << " w: "<< obj->width << " h: " << obj->height << std::endl;
    ...
    std::cout << " 3D points size: "<< cloud->points.size() << std::endl;
    ...
    std::cout << " 2D points size: "<< cloud_xy->points.size() << std::endl;
    pcl::io::savePCDFileASCII("/home/zb/dataset/435/001/cloud_xy.pcd", *cloud_xy);
    ...
    std::cout << " polygon: " << obj->polygon << std::endl;
...
void MinBoxObjectBuilder::reconstructPolygon3d(ObjectPtr obj) {
    ...
    std::ofstream file;
    file.open("/home/gxc/dataset/ObjectDection/polygon_info.txt");
    file << "All Points: " << std::endl << "x: "<< std::endl;

    cv::Mat plane = cv::Mat(3200,3200, CV_8UC3, cv::Scalar(255, 255, 255));
    for(int i = 0; i < obj->polygon.points.size(); i++){
//        cv::Point p;
//        p.x = abs(obj->polygon[i].x) * 1000 + 500;
//        p.y = obj->polygon[i].y * 1000 -1000;
//        std::cout << "p: " << p << std::endl;
//        cv::circle(plane, p, 5, cv::Scalar(0, 0, 255), -1);

        file << obj->polygon[i].x << ",";
    }

    file << std::endl << "y: "<< std::endl;
    for(int i = 0; i < obj->polygon.points.size(); i++){
        file << obj->polygon[i].y << ",";
    }
    file << std::endl;
//    cv::imwrite("/home/gxc/dataset/ObjectDection/polygon_point.png", plane);
    ...
    std::cout << "min_point_index: " << min_point_index << std::endl;
    std::cout << "max_point_index: " << max_point_index << std::endl;
    cv::Point max_p, min_p;
//    max_p.x = abs(obj->polygon[max_point_index].x) * 1000 + 500;
//    max_p.y = obj->polygon[max_point_index].y * 1000 -1000;
//    min_p.x = abs(obj->polygon[min_point_index].x) * 1000 + 500;
//    min_p.y = obj->polygon[min_point_index].y * 1000-1000;

//    line(plane,max_p, min_p, cv::Scalar(255, 0, 0));

//    cv::imwrite("/home/gxc/dataset/ObjectDection/polygon_line.png", plane);

    file << std::endl << "Max Point: " << std::endl;
    file <<"[" << obj->polygon[max_point_index].x << "," << obj->polygon[max_point_index].y << "]" << std::endl;
    file << std::endl << "Min Point: " << std::endl;
    file <<"[" << obj->polygon[min_point_index].x << "," << obj->polygon[min_point_index].y << "]" << std::endl;


    Eigen::Vector3d line = max_point - min_point;
    double line_legth = sqrt((max_point[0] - min_point[0]) * (max_point[0] - min_point[0]) +
                               (max_point[1] - min_point[1]) * (max_point[1] - min_point[1]));

    std::cout << "line_legth: " << line_legth << std::endl;
    ...
    for (size_t i = min_point_index, count = 0;
        ...
        if (j != min_point_index && j != max_point_index) {
            ...
            if (line[0] * ray[1] - ray[0] * line[1] < common::EPSILON) {
                std::cout << " < common::EPSILON " << std::endl;
                std::cout << "i: " << i << "  j: " << j<< std::endl;
            ...
            if (line[0] * ray[1] - ray[0] * line[1] < common::EPSILON) {
                ...
                std::cout << " j == min_point_index || j == max_point_index "  << std::endl;
                std::cout << "i: " << i << "  j: " << j<< std::endl;
                std::cout << " max_dis: " << max_dis << std::endl;
                std::cout << " total_len: " << total_len << std::endl;
    ...
    std::cout << " final max_dis: " << max_dis << std::endl;
    std::cout << " total legth: " << total_len << std::endl;
    ...
    for (size_t i = min_point_index; count < obj->polygon.points.size();
        ...
        std::cout << " Step3: out continue  "  << std::endl;
        std::cout << "i: " << i << "  j: " << j<< std::endl;
    ...
    double my_legth = 0;
    for(int i = 7; i < 15; i++){
        double single_legth = sqrt((obj->polygon.points[i].x - obj->polygon.points[i+1].x) * (obj->polygon.points[i].x - obj->polygon.points[i+1].x) +
                                  (obj->polygon.points[i].y - obj->polygon.points[i+1].y) * (obj->polygon.points[i].y - obj->polygon.points[i+1].y));

        std::cout <<  "  my_legth: " << my_legth << std::endl;
        my_legth += single_legth;
    }

    my_legth +=  sqrt((obj->polygon.points[15].x - obj->polygon.points[0].x) * (obj->polygon.points[15].x - obj->polygon.points[0].x) +
                      (obj->polygon.points[15].y - obj->polygon.points[0].y) * (obj->polygon.points[15].y - obj->polygon.points[0].y));

    double longest_line = sqrt((obj->polygon.points[15].x - obj->polygon.points[14].x) * (obj->polygon.points[15].x - obj->polygon.points[14].x) +
                               (obj->polygon.points[15].y - obj->polygon.points[14].y) * (obj->polygon.points[15].y - obj->polygon.points[14].y));

    std::cout <<  " longest_line: " << longest_line << std::endl;
    std::cout <<  " final my_legth: " << my_legth << std::endl;
...
//        obj->direction = dir;
//        min_area = area;
//        std::cout << "i: " << i << "  area: " << min_area << std::endl;
//    }


//    double area = computeAreaAlongOneEdge(obj, 7, &center, &length,
//                                                  &width, &dir);
//    obj->ground_center = center;
//    obj->length = length;
//    obj->width = width;
...
//    min_area = area;
    ...
    rect[0] = cv::Point2f((obj->ground_center[0] - obj->length/2.0),
            (obj->ground_center[1] - obj->width/2.0) );

    rect[1] = cv::Point2f((obj->ground_center[0] + obj->length/2.0) ,
                                (obj->ground_center[1] - obj->width/2.0) );

    rect[2] = cv::Point2f((obj->ground_center[0] + obj->length/2.0) ,
                                (obj->ground_center[1] + obj->width/2.0));

    rect[3] = cv::Point2f((obj->ground_center[0] - obj->length/2.0) ,
                                (obj->ground_center[1] + obj->width/2.0) );

    file << std::endl << "Rect Points: " << std::endl;
    file <<"[";
    for(int i=0; i< rect.size(); i++){
        file << rect[i].x << "," ;

//        cv::line(plane,rect[i], rect[i+1], cv::Scalar(0, 255, 0));
//        cv::circle(plane, rect[i], 5, cv::Scalar(0, 255, 0), -1);
    }
    file <<"]" << std::endl;
    file <<"[";
    for(int i=0; i< rect.size(); i++){
...
//    cv::imwrite("/home/gxc/dataset/ObjectDection/polygon_rect.png", plane);

    // <4> 边框方向
    obj->direction.normalize();

    file <<"Rect Direction: " << std::endl;
    file << obj->direction << std::endl;
    //by sun for rect show
    double theta = 0;
    if(obj->direction[0] != 0)
    {
        theta = atan(obj->direction[1] / obj->direction[0]);
    }
    else{
        theta = 3.1415 / 2;
    }

    Eigen::Matrix3d t_transform = Eigen::Matrix3d::Identity();
    Eigen::Matrix3d t_transform_2 = Eigen::Matrix3d::Identity();

//    Eigen::Matrix3d r_transform = Eigen::Matrix3d::Identity();
    Eigen::Isometry2d r_transform = Eigen::Isometry2d::Identity();

    Eigen::Matrix3d transform_label = Eigen::Matrix3d::Identity();

    std::cout << " theta : " << theta << std::endl;

//    theta = 3.1415 / 2;

    t_transform(0, 2) = - obj->ground_center[0];
    t_transform(1, 2) = - obj->ground_center[1];

    t_transform_2(0, 2) = obj->ground_center[0];
    t_transform_2(1, 2) = obj->ground_center[1];

    file << std::endl <<"t_Transform: " << std::endl;
    file << t_transform.cast<double>().matrix() << std::endl;

    std::cout << cos(theta) << " " << -sin(theta) << " " << sin(theta) << " " << cos(theta) << std::endl;

    r_transform(0, 0) = cos(theta);
    r_transform(0, 1) = -sin(theta);
    r_transform(1, 0) = sin(theta);
    r_transform(1, 1) = cos(theta);
    r_transform(0, 2) = 0;
    r_transform(1, 2) = 0;

//    transform(0, 2) = obj->ground_center[0];
//    transform(1, 2) = obj->ground_center[1];

//    Eigen::Isometry2d a2f_transform(r_transform.cast<double>());
//    std::cout << "a2f_transform = \n" << a2f_transform.matrix() << std::endl;

    file << std::endl <<"r_Transform: " << std::endl;
    file << r_transform.matrix() << std::endl;

    Eigen::MatrixXd rect_corners(3, 4);
    Eigen::MatrixXd rect_corners_sys(3, 4);

    //   1-------2
    //  /       /
    // 4-------3

    rect_corners <<  rect[0].x, rect[1].x, rect[2].x, rect[3].x,
                     rect[0].y, rect[1].y, rect[2].y, rect[3].y,
                           1.0,       1.0,       1.0,       1.0;
    std::cout << "transform: " << t_transform << std::endl;
    rect_corners_sys = t_transform_2 * r_transform * t_transform * rect_corners;

    file << std::endl <<"Rect Corners: " << std::endl;
    file << rect_corners << std::endl;
    file << std::endl <<"Rect Corners in Sys: " << std::endl;
    file << rect_corners_sys << std::endl;
